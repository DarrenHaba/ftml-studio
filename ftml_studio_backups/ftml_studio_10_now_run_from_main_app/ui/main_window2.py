...
from PySide6.QtWidgets import (QVBoxLayout, QHBoxLayout,
                               ...
                               QComboBox, QApplication, QFrame, QSplitter)
...
class MainWindow(BaseWindow):
    ...
    def setup_ui(self):
        ...
        # Create a splitter for content and schema
        ...
        # Schema pane (will be initially hidden)
        self.schema_widget = QWidget()
        self.schema_layout = QVBoxLayout(self.schema_widget)
        self.schema_layout.setContentsMargins(10, 10, 10, 10)

        # Add dummy content to schema widget
        schema_label = QLabel("Schema View")
        schema_label.setAlignment(Qt.AlignCenter)
        schema_label.setFont(QFont("Arial", 14, QFont.Bold))
        self.schema_layout.addWidget(schema_label)

        # Add placeholder text
        schema_content = QLabel("This is where the schema editor will appear.\nIt will show the structure of your FTML document.")
        schema_content.setAlignment(Qt.AlignCenter)
        schema_content.setWordWrap(True)
        self.schema_layout.addWidget(schema_content)

        # Add main content and schema to splitter
        self.main_splitter.addWidget(self.content_widget)
        self.main_splitter.addWidget(self.schema_widget)

        # Set initial sizes - hide schema by default
        self.main_splitter.setSizes([1, 0])

        # Add splitter to main layout
        self.main_layout.addWidget(self.main_splitter, 1)  # Stretch factor 1
    ...
    def create_sidebar(self):
        ...
        self.schema_btn = QPushButton("⋮⋮")  # Unicode vertical dots symbol for schema
        self.schema_btn.setObjectName("sidebarButton")
        self.schema_btn.setFixedSize(30, 30)
        self.schema_btn.setToolTip("Toggle Schema View")
        self.schema_btn.clicked.connect(self.toggle_schema_panel)

        # Add buttons to sidebar
        sidebar_layout.addWidget(self.settings_btn)
        sidebar_layout.addWidget(self.schema_btn)
    ...
    def toggle_schema_panel(self):
        """Toggle the visibility of the schema panel"""
        self.schema_visible = not self.schema_visible
        logger.debug(f"Toggling schema panel: {self.schema_visible}")

        # Get current sizes
        sizes = self.main_splitter.sizes()

        if self.schema_visible:
            # Show schema - allocate 30% of width to schema
