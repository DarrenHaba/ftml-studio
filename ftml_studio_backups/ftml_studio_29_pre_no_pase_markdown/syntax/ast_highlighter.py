...
class FTMLASTHighlighter(BaseHighlighter):
    ...
    def apply_fallback_highlighting(self, text):
        ...
        # Single-quoted strings (with double-quote escaping only, no backslash escapes)
        # FIXED: In FTML, single quotes only escape with '' not with backslash
    ...
    def highlight_value_node(self, node, text):
        ...
        if isinstance(node.value, str):
            ...
            # Find double-quoted strings with escape sequences
            double_quote_pattern = QRegularExpression(r'"(?:\\.|[^"\\])*"')

            # Find single-quoted strings with only '' as escape
            single_quote_pattern = QRegularExpression(r"'(''|[^'])*'")

            # Apply formatting for double-quoted strings
            ...
            # Apply formatting for single-quoted strings
