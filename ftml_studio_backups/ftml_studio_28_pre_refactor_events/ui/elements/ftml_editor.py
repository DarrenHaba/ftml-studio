...
class FTMLEditorWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.editor.setHighlighterParent(self)
    ...
    def _apply_delayed_selections(self):
        """Apply the delayed selections after timer expires"""
        if hasattr(self, '_pending_selections') and self.error_highlighting_enabled:
            logger.debug(f"Applying {len(self._pending_selections)} delayed selections")
            self.editor.setExtraSelections(self._pending_selections)

    def set_extra_selections_delayed(self, selections):
        """Store selections to be applied after delay"""
        if not self.error_highlighting_enabled:
            # If error highlighting is disabled, just clear selections
            self.editor.setExtraSelections([])
            return
    
        # Store selections for delayed application
        ...
        # Start timer for delayed application if delay > 0
        if self.error_highlight_delay > 0:
            logger.debug(f"Scheduling error highlight application with {self.error_highlight_delay}ms delay")
            self.error_highlight_timer.start(self.error_highlight_delay)
        else:
            # Apply immediately if delay is 0
            self._apply_delayed_selections()
