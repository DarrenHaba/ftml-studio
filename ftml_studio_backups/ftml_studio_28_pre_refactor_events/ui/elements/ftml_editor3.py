...
class EditorSettingsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.highlight_checkbox = QCheckBox("Show Error Indicators")
        ...
        self.parse_delay_slider.setMaximum(2000)  # 2 seconds in ms
        self.parse_delay_slider.setValue(self.parse_delay)
        self.parse_delay_slider.setTickInterval(100)
...
class FTMLEditorWidget(QWidget):
    ...
    def show_settings_dialog(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            # Apply any pending error highlights with new delay
            if self.pending_errors:
                self.schedule_error_highlights(self.pending_errors)

            logger.debug(f"Applied settings: error_highlight={self.error_highlighting_enabled}, "
                         f"error_delay={self.error_highlight_delay}ms, parse_delay={self.parse_delay}ms")
    ...
    def update_error_display(self, errors):
        ...
        if (self.error_highlighting_enabled and hasattr(self.highlighter, 'error_highlighting') and
                self.highlighter.error_highlighting):
    ...
    def apply_delayed_error_highlights(self):
        ...
        if self.pending_errors and self.error_highlighting_enabled:
