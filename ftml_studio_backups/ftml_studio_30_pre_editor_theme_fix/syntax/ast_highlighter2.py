...
class FTMLASTHighlighter(BaseHighlighter):
            ...
            for elem in node.elements:
                ...
                if hasattr(elem, 'line') and elem.line == block_number:
                    ...
                    if isinstance(elem, ScalarNode) and isinstance(elem.value, str):
                        ...
                        logger.debug(f"Processing non-string list element")
    ...
    def highlight_value_node(self, node, text):
        ...
        # CRITICAL: Find equals sign first to only highlight values, not keys
        equals_pos = text.find("=")
        if equals_pos < 0:
            logger.debug("No equals sign found in text")
            return

        # Only search for values AFTER the equals sign
        ...
        if isinstance(node.value, str):
            ...
            # Find double-quoted strings, but ONLY after equals sign
            double_quote_pattern = QRegularExpression(r'"(?:\\.|[^"\\])*"')
            match = double_quote_pattern.match(text, search_start)
            if match.hasMatch():
                self.setFormat(match.capturedStart(), match.capturedLength(), self.formats["string"])
                logger.debug(f"Highlighted double-quoted string at {match.capturedStart()}")
                return

            # Find single-quoted strings, but ONLY after equals sign
            single_quote_pattern = QRegularExpression(r"'(''|[^'])*'")
            match = single_quote_pattern.match(text, search_start)
            if match.hasMatch():
                self.setFormat(match.capturedStart(), match.capturedLength(), self.formats["string"])
                logger.debug(f"Highlighted single-quoted string at {match.capturedStart()}")
                return

        # 2. Boolean values
        elif isinstance(node.value, bool):
            # Find boolean value in the text, only after equals sign
            value_str = str(node.value).lower()  # FTML uses lowercase true/false
            logger.debug(f"Processing boolean value: {value_str}")
            pos = text.find(value_str, search_start)
            if pos >= 0:
                logger.debug(f"Highlighting boolean at position {pos}")
                self.setFormat(pos, len(value_str), self.formats["boolean"])
            else:
                logger.debug(f"Boolean '{value_str}' not found in text after equals")

        # 3. Null values
        elif node.value is None:
            # Find "null" in the text, only after equals sign
            logger.debug("Processing null value")
            pos = text.find("null", search_start)
            if pos >= 0:
                logger.debug(f"Highlighting null at position {pos}")
                self.setFormat(pos, 4, self.formats["null"])
            else:
                logger.debug("'null' not found in text after equals")

        # 4. Number values - check last so it doesn't try to find booleans as numbers
        elif isinstance(node.value, (int, float)):
            # Find the number in the text, only after equals sign
            ...
            if pos >= 0:
                ...
                logger.debug(f"Number '{value_str}' not found in text after equals")
