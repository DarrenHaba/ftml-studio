...
class MainWindow(BaseWindow):
    ...
    def setup_settings_panel(self):
        ...
        # Connect the parse settings change signal
        self.settings_panel.parseSettingChanged.connect(self.on_parse_settings_changed)
    
        # Add to content widget
        self.content_widget.addWidget(self.settings_panel)

    
    def on_parse_settings_changed(self):
        """Handle changes to parsing settings"""
        logger.debug("Parse settings changed")
    
        # Apply the new settings to all FTML editors
        self.apply_parse_settings()
    
        # Show status message
        auto_parse = self.settings_panel.get_auto_parse_setting()
        parse_delay = self.settings_panel.get_parse_delay_setting() / 1000  # Convert to seconds
        if auto_parse:
            self.statusBar().showMessage(f"Auto-parsing enabled with {parse_delay:.1f}s delay", 3000)
        else:
            self.statusBar().showMessage("Auto-parsing disabled", 3000)


    def apply_parse_settings(self):
        """Apply the auto-parse and parse delay settings to all editor components"""
        # Get current settings
        auto_parse = self.settings_panel.get_auto_parse_setting()
        parse_delay = self.settings_panel.get_parse_delay_setting()
    
        logger.debug(f"Applying parse settings: auto_parse={auto_parse}, parse_delay={parse_delay}ms")
    
        # Apply to editor widget
        if hasattr(self, 'editor_widget'):
            if hasattr(self.editor_widget, 'highlighter'):
                # Update parse delay
                self.editor_widget.update_parse_settings(auto_parse, parse_delay)
                self.editor_widget.highlighter.parse_delay = parse_delay
    
                # If auto-parse is disabled, remove timer
                if not auto_parse and hasattr(self.editor_widget.highlighter, 'parse_timer'):
                    self.editor_widget.highlighter.parse_timer.stop()
    
                # Also update error highlighting (will be disabled if auto-parse is off)
                self.apply_error_indicators_setting()
    
                logger.debug(f"Applied parse settings to editor")
    
        # Apply to converter widget
        if hasattr(self, 'converter_widget'):
            # Update any FTML highlighters in the converter
            if hasattr(self.converter_widget, 'source_highlighter') and hasattr(self.converter_widget.source_highlighter, '__class__'):
                if self.converter_widget.source_highlighter.__class__.__name__ == 'FTMLASTHighlighter':
                    self.converter_widget.source_highlighter.parse_delay = parse_delay
                    if not auto_parse and hasattr(self.converter_widget.source_highlighter, 'parse_timer'):
                        self.converter_widget.source_highlighter.parse_timer.stop()
                    logger.debug(f"Applied parse settings to converter source editor")
    
            if hasattr(self.converter_widget, 'target_highlighter') and hasattr(self.converter_widget.target_highlighter, '__class__'):
                if self.converter_widget.target_highlighter.__class__.__name__ == 'FTMLASTHighlighter':
                    self.converter_widget.target_highlighter.parse_delay = parse_delay
                    if not auto_parse and hasattr(self.converter_widget.target_highlighter, 'parse_timer'):
                        self.converter_widget.target_highlighter.parse_timer.stop()
                    logger.debug(f"Applied parse settings to converter target editor")

        # Force rehighlight all editors to refresh the highlighting
        QTimer.singleShot(200, self.refresh_all_highlighting)

    def refresh_all_highlighting(self):
        """Force rehighlight all editors to refresh syntax highlighting"""
        logger.debug("Refreshing syntax highlighting in all editors")
    
        # Refresh editor widget
        if hasattr(self, 'editor_widget') and hasattr(self.editor_widget, 'highlighter'):
            self.editor_widget.highlighter.rehighlight()
    
        # Refresh converter highlighters
        if hasattr(self, 'converter_widget'):
            if hasattr(self.converter_widget, 'source_highlighter'):
                self.converter_widget.source_highlighter.rehighlight()
            if hasattr(self.converter_widget, 'target_highlighter'):
    ...
    def apply_error_indicators_setting(self):
        ...
        # Get the current settings from settings panel
        if hasattr(self, 'settings_panel'):
            ...
            logger.debug(f"Applying error indicators setting: {enabled} (auto_parse={auto_parse})")
