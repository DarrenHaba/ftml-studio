...
class FTMLEditorWidget(QWidget):
    ...
    def update_parse_settings(self, auto_parse_enabled=None, parse_delay=None):
        ...
        if not auto_parse_enabled and hasattr(self.highlighter, 'parse_timer'):
            ...
            # IMPORTANT: Still do a one-time parse for syntax highlighting
            QTimer.singleShot(100, self.highlighter.parse_document)
            logger.debug("Scheduled a one-time parse for initial syntax highlighting")
        ...
        # If auto-parse is enabled but was previously disabled, trigger a parse now
            ...
            # Schedule a one-time parse
            QTimer.singleShot(100, self.highlighter.parse_document)
            self.highlighter.parse_timer.start(parse_delay)  # Then start the timer for continuous parsing
