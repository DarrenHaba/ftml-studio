...
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout,
                               ...
                               QMessageBox, QCheckBox, QComboBox, QMainWindow)
...
class FTMLEditorWidget(QWidget):
    ...
    def setup_menu_bar(self, main_layout):
        ...
        # Set stylesheet for menu bar to ensure visibility in both themes
        is_dark = theme_manager.get_active_theme() == theme_manager.DARK
        self.menu_bar.setStyleSheet(self.get_menu_style(is_dark))
    ...
    def get_menu_style(self, is_dark):
    ...
    def recreate_highlighter(self):
        ...
        # Update menu bar styling for theme change
        ...
        if hasattr(self, 'menu_bar'):
            is_dark = theme_manager.get_active_theme() == theme_manager.DARK
            style = self.get_menu_style(is_dark)
            logger.debug(f"Applying {'dark' if is_dark else 'light'} menu style to menu_bar: {style[:50]}...")
            self.menu_bar.setStyleSheet(style)
            self.menu_bar.update()  # Force update
