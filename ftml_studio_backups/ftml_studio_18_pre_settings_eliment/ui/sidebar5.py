...
class ThemedIcon:
    ...
    def load(icon_name, parent=None, is_dark_theme=False):
        ...
        print(f"DEBUG: Loading icon '{icon_name}' with is_dark_theme={is_dark_theme}")
        ...
        print(f"DEBUG: Selected icon folder: '{folder}'")
        ...
        print(f"DEBUG: Full icon path: '{icon_path}'")
        print(f"DEBUG: Does icon exist at path? {os.path.exists(icon_path)}")
        ...
        if not os.path.exists(icon_path):
            ...
            print(f"DEBUG: Does fallback exist? {os.path.exists(fallback_path)}")
    
            if os.path.exists(fallback_path):
                print(f"DEBUG: Using fallback icon at: {fallback_path}")
            ...
            if parent:
                print(f"DEBUG: Using standard icon as fallback for: {icon_name}")
        ...
        print(f"DEBUG: Successfully loading themed icon from: {icon_path}")
...
class Sidebar(QWidget):
    ...
    def toggle_expansion(self):
        ...
        print(f"DEBUG: Toggling sidebar. Current expanded state: {self.expanded}")
        ...
        is_dark = theme_manager.get_active_theme() == theme_manager.DARK
        print(f"DEBUG: Theme is {'DARK' if is_dark else 'LIGHT'}")
    
        # Use the themed icon for menu open/close
        icon_name = "menu_close" if self.expanded else "menu_open"
        print(f"DEBUG: Loading hamburger icon: {icon_name}")
        ...
        if not self.expanded:
            print(f"DEBUG: Collapsing sidebar - removing button texts")
            for btn in [self.editor_btn, self.converter_btn, self.settings_btn]:
                btn.setText("")
                # Apply styling
                is_dark = theme_manager.get_active_theme() == theme_manager.DARK
        ...
        print(f"DEBUG: Animation target width: {target_width}")
        ...
        if self.expanded:
            print(f"DEBUG: Connecting add_button_texts to animation finished")
        ...
        print(f"DEBUG: Sidebar toggle complete. New expanded state: {self.expanded}")
    ...
    def update_theme(self):
        ...
        for btn in [self.editor_btn, self.converter_btn, self.settings_btn, self.hamburger_btn]:
            print(f"DEBUG: Updating button icon: {btn.icon_name if hasattr(btn, 'icon_name') else 'hamburger'}")
