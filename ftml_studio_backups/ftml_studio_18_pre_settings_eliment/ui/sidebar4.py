...
DARK_THEME_ICON_COLOR = QColor(255, 255, 255)  # White for dark theme
LIGHT_THEME_ICON_COLOR = QColor(0, 0, 0)       # Black for light theme
...
class ThemedIcon:
    ...
    def load(icon_name, parent=None, is_dark_theme=False):
        """
        Load an SVG icon and apply appropriate theme coloring
        ...
        Returns:
            QIcon: Properly colored icon for the current theme
        """
        # Path to the SVG icon file
        icon_path = os.path.join(os.path.dirname(__file__), f"icons/{icon_name}.svg")

        if not os.path.exists(icon_path):
            logger.warning(f"SVG icon not found at: {icon_path}")

            # Try PNG as fallback
            fallback_path = os.path.join(os.path.dirname(__file__), f"icons/{icon_name}.png")
            if os.path.exists(fallback_path):
                logger.debug(f"Using PNG fallback icon at: {fallback_path}")
                return QIcon(fallback_path)

            # Return a default icon if no icon exists
            if parent:
                logger.warning(f"Using standard icon as fallback for: {icon_name}")
                return parent.style().standardIcon(parent.style().SP_FileIcon)
            return QIcon()

        # Choose icon color based on theme
        if is_dark_theme:
            icon_color = DARK_THEME_ICON_COLOR
        else:
            icon_color = LIGHT_THEME_ICON_COLOR

        # Method 1: XML manipulation to change the fill color
        try:
            ...
            # Find all path elements (Google Material icons typically use path elements)
            # Use explicit namespaces for SVG to avoid parsing issues
            namespaces = {'svg': 'http://www.w3.org/2000/svg'}
            paths = root.findall('.//svg:path', namespaces) or root.findall('.//path')

            # Update the fill attribute for all paths
            for path in paths:
                # Set the fill color
                path.set('fill', icon_color.name())

            # Convert back to string
            ET.register_namespace('', 'http://www.w3.org/2000/svg')
            modified_svg = ET.tostring(root, encoding='utf-8')

            # Create a QIcon from the modified SVG
            from PySide6.QtSvg import QSvgRenderer
            from PySide6.QtCore import QByteArray, QBuffer, QIODevice

            # Create a renderer for the SVG
            svg_bytes = QByteArray(modified_svg)
            renderer = QSvgRenderer(svg_bytes)

            # Create a pixmap to render onto (with appropriate device pixel ratio)
            device_pixel_ratio = parent.devicePixelRatio() if parent else 1.0
            size = 24 * device_pixel_ratio
            ...
            logger.debug(f"Successfully loaded and colored SVG icon: {icon_name}")
            return QIcon(pixmap)

        except Exception as e:
            logger.error(f"Error processing SVG with XML method: {str(e)}")

            # Try fallback method (QPainter colorization)
            try:
                from PySide6.QtSvg import QSvgRenderer
                ...
                # Paint the original pixmap with the theme color
                ...
                logger.debug(f"Successfully colored SVG icon using fallback method: {icon_name}")
                return QIcon(colored_pixmap)

            except Exception as e:
                logger.error(f"Error in fallback SVG coloring: {str(e)}")
...
class Sidebar(QWidget):
    ...
    def toggle_expansion(self):
        ...
        # Use the themed icon for menu open/close
        icon_name = "menu_close" if self.expanded else "menu_open"
        self.hamburger_btn.icon_name = icon_name  # Update stored icon name
        self.hamburger_btn.setIcon(ThemedIcon.load(icon_name, self, is_dark))
    ...
    def update_theme(self):
        ...
        is_dark = theme_manager.get_active_theme() == theme_manager.DARK
        print(f"DEBUG: Theme update triggered. Active theme is{'DARK' if is_dark else 'LIGHT'}")
        print(f"DEBUG: Sidebar expanded state: {self.expanded}")
        ...
        for btn in [self.editor_btn, self.converter_btn, self.settings_btn]:
            print(f"DEBUG: Updating button icon: {btn.icon_name}")
            if hasattr(btn, 'update_theme'):
                btn.update_theme(is_dark)
    
        print(f"DEBUG: Theme update completed")
